...
contract PayPool is ReentrancyGuard {
    ...
    event DepositStatusChanged(uint256 indexed index, DepositStatus newStatus);
    ...
    function approveDeposit(uint256 index) external isOwner {
        ...
        require(depositHistory[index].status == DepositStatus.Pending, "Deposit is not pending");
